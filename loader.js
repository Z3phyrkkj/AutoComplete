...
(function() {
            ...
            return [...questsMap.values()].filter(quest => {
                try {
                    return quest.userStatus?.enrolledAt &&
                        ...
                        window.QuestHelperConfig.supportedTasks.some(task => {
    ...
    function refreshQuests(elements) {
        ...
        window.QuestHelperUI.updateQuestList(state.quests, window.QuestHelperUI.taskIcons, window.QuestHelperUtils);
        
        window.QuestHelperUtils.showMessage(
    ...
    function handleCallback(type, data, elements) {
        switch (type) {
            case 'message':
                window.QuestHelperUtils.showMessage(data.text, data.type, { text: elements.statusText, dot: elements.statusDot });
                break;
            case 'refresh':
    ...
    window.QuestHelperUI.injectStyles();

    const navIcon = window.QuestHelperUI.createNavIcon(() => {
    ...
    const elements = window.QuestHelperUI.createPanel(
        () => {
        ...
        () => {
            refreshQuests(elements);
            window.QuestHelperUtils.showMessage('Lista atualizada', 'success', { text: elements.statusText, dot: elements.statusDot });
        },
        () => {
    ...
    refreshQuests(elements);
    state.refreshTimer = setInterval(() => refreshQuests(elements), window.QuestHelperConfig.refreshInterval);
